...
- `npm run test:sondrik` — Sondrik's own regression tests for its shared date-math (`goals-core.js`, `release-core.js`, `validate-core.js`); run after touching any of them
- `npm run test:cgt` — CGT's regression tests for its grading-ROI/collectibles-tax/card-value math (`grading-core.js`), grading-turnaround/return-date math (`turnaround-core.js`), and cards/submissions/candidates validation rules (`validate-core.js`); run after touching any of them
...
- `npm run test:dashboard` — the hub page's own regression tests for its staleness/grid-sort math (`dashboard-core.js`); run after touching that file
