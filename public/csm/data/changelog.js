...
function run() {
  ...
  const entries = log.split(RECORD_SEP).map(block => block.trim()).filter(Boolean).map(block => {
    ...
    // has nothing to do with prospects.json/stages.json (seen in practice:
    // a root commit here with a CGT mobile-overflow subject). Showing that
    ...
    // with an honest note instead of the misleading original subject.
