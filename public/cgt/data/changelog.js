...
function run() {
  ...
  const entries = log.split(RECORD_SEP).map(block => block.trim()).filter(Boolean).map(block => {
    ...
    // has nothing to do with cards.json/submissions.json/candidates.json
    // (seen in practice: a root commit here with an unrelated server.js
    // subject). Showing that subject as this file's real change history
    // would be exactly the kind of unverified claim this changelog exists
    // to avoid, so it's replaced with an honest note instead of the
    // misleading original subject. Same fix already landed on the CSM and
    // Sondrik hubs for the identical bug.
