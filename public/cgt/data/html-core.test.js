...
test('escapeHtml leaves an ordinary value untouched', () => {
  assert.equal(escapeHtml('2018 Prizm'), '2018 Prizm');
});
...
test('escapeHtml neutralizes a script tag rather than letting it render as live markup', () => {
  // Real XSS guard (OWASP): every hub renders hand-editable JSON field
  // values straight into innerHTML, so a card name or comp note containing
